 ...
 This link should be displayed to the user.",
  {
    ...
    dueDate: z.string().describe("The due date of the invoice (YYYY-MM-DD format). Defaults to 30 days from today if not provided.").optional(),
    currencyCode: z.string().describe("The currency code for the invoice, e.g. AUD, USD, GBP. Defaults to the organisation's base currency if not provided.").optional(),
    lineAmountTypes: z.enum(["EXCLUSIVE", "INCLUSIVE", "NOTAX"]).describe("How tax is applied to line items. EXCLUSIVE = amounts are tax exclusive, INCLUSIVE = amounts are tax inclusive, NOTAX = no tax applied.").optional(),
    brandingThemeId: z.string().describe("The ID of the Xero branding theme to apply to the invoice. Can be obtained from the list-branding-themes tool.").optional(),
  },
  async ({ contactId, lineItems, type, reference, date, dueDate, currencyCode, lineAmountTypes, brandingThemeId }) => {
    const xeroInvoiceType = type === "ACCREC" ? Invoice.TypeEnum.ACCREC : Invoice.TypeEnum.ACCPAY;
    const result = await createXeroInvoice(contactId, lineItems, xeroInvoiceType, reference, date, dueDate, currencyCode, lineAmountTypes, brandingThemeId);
    if (result.isError) {
      return {
        content: [
          {
